...
void loopStable(RTIMU *imu) {
	...
	if(std::abs(getRoll(imu)) >= std::abs(64/getRightwardVelocity(imu))) {
	...
	if(std::abs(getPitch(imu)) >= std::abs(64/getForwardVelocity(imu))) {
	...
	if(!dryrun) { 
		...
		std::cout <<  std::abs(64/getRightwardVelocity(imu)) << std::endl;
